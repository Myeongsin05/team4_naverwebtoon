...
public class AuthorHome extends Author {
	...
	*/
	...
	public void showPostList() { // 등록된 포스트을 보여주는 메서드
		...
		if (dir.exists() && dir.isDirectory()) { // dir이 존재하고 dir이 디렉토리 형식인지 확인
			String[] postList = dir.list();
			if (postList != null) { // 포스트리스트가 비어 있지 않으면
				for (String post : postList) {
					System.out.println(post); // 디렉토리 내에 있는 파일명 보여주기
				}
			}else System.out.println("등록된 포스트가 없습니다."); // 디렉토리에 파일이 없을 때
		}else System.out.println("포스트 디렉토리가 존재하지 않습니다.");
	}
	...
	public void deletePost() { // 포스트 삭제 메서드
		...
		if (deleteFile.exists()) {
			...
			System.out.println("존재하지 않은 포스트명이니다.");
		/*
